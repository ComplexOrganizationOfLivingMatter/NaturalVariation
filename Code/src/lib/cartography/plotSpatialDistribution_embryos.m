function plotSpatialDistribution_embryos(rgStackPath, labelsPath, variable, savePath, saveName, minMaxRanges)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % plotSpatialDistribution_embryos
...
%%  this part is commented since i am not sure if its needed
...
%         labelledImage = imgRelabel;
        ...
        validCells_size = find(table2array(regionprops3(labelledImage_BASAL,'Volume'))>0);
        ...
        noValidCells = setdiff(unique(labelledImage_BASAL), validCells);
        ...
        elseif contains(variable,"NumNeighs")
            uniqueLabels = unique(labelledImage_BASAL);
            unqueLabels = uniqueLabels(find(uniqueLabels~=0));
            colours = [];
            maxValue = 9;
            minValue = 3;
